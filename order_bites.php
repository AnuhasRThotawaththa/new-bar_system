...
<body class="bg-black/80 text-white relative">
  ...
  <section class="py-12 text-center max-w-6xl mx-auto px-4 relative z-10">
    <h1 class="text-3xl font-bold text-yellow-400 mb-6">Our Bites Collection 🍽️</h1>
    <p class="text-gray-300 max-w-3xl mx-auto mb-10 text-sm">Pair your drinks with these perfect pub snacks and bites.</p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10">
      ...
      $query = "SELECT * FROM bites ORDER BY id ASC";
      $result = pg_query($conn, $query);

      if (!$result) {
        echo "<p class='text-red-500'>Failed to fetch bites.</p>";
      } else {
        while ($bite = pg_fetch_assoc($result)):
          $safeId = safeId($bite['name']);
      ?>
        <div class="bg-gray-900/50 backdrop-blur-sm rounded-xl shadow-lg p-5 flex flex-col hover:scale-105 hover:shadow-yellow-400 transition-all duration-300">
          <div class="w-full h-52 mb-4 rounded-lg bg-white flex items-center justify-center shadow-inner">
            <img src="<?= htmlspecialchars($bite['image']) ?>" alt="<?= htmlspecialchars($bite['name']) ?>" class="max-h-48 object-contain" />
          </div>
          <h3 class="text-lg font-semibold text-yellow-300 mb-1"><?= htmlspecialchars($bite['name']) ?></h3>
          <p class="text-gray-300 text-sm mb-2"><?= htmlspecialchars($bite['description']) ?></p>
          <span class="text-yellow-400 font-bold text-sm mb-3">Rs. <?= number_format($bite['price']) ?></span>
          ...
          <button id="btn_<?= $safeId ?>" onclick="addBiteToCart(<?= $bite['id'] ?>, '<?= $safeId ?>')" class="bg-yellow-400 text-black font-semibold py-2 rounded hover:bg-yellow-300 transition-all duration-300">Add to Cart</button>
        </div>
      <?php endwhile; } ?>
    </div>
  ...
  <footer class="bg-gray-950/80 backdrop-blur-sm text-center py-4 mt-10 text-xs text-gray-300 z-10 relative">
    ...
    <p>Crafted with passion. Eat and drink responsibly.</p>
  </footer>
  ...
  <script>
    ...
    function addBiteToCart(biteId, inputId) {
      const qtyInput = document.getElementById('qty_' + inputId);
      ...
      xhr.open("POST", "add_bite_to_cart.php", true);
      ...
      const params = `bite_id=${biteId}&qty=${qty}`;
      xhr.send(params);
    }
  </script>
</body>
